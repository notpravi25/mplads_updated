...
export async function fetchRiskQueue(params: {
  ...
  if (params.min_vendor_risk !== undefined) query.append('min_vendor_risk', params.min_vendor_risk.toString());
...
export async function fetchWorkDetail(workId: string): Promise<{ work: WorkRecord; candidate_duplicates: CandidateDuplicatePair[] }> {
  ...
  return safeFetchJson<{ work: WorkRecord; candidate_duplicates: CandidateDuplicatePair[] }>(`${API_BASE}/work-detail?${query.toString()}`, { work: {} as WorkRecord, candidate_duplicates: [] });
}

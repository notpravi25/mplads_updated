...
import { Sparkles, X, Send, Bot, User, ArrowRight, ShieldAlert, PieChart, Building2, Copy, CheckCircle2 } from 'lucide-react';
...
export const AiAssistantModal: React.FC<AiAssistantModalProps> = ({
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      sender: 'assistant',
      text: "Hello! I am your MPLADS AI Risk Copilot. I analyze real-time expenditure data, physical-financial progress gaps, vendor concentration patterns, duplicate work candidates, and compliance evidence gaps across 79,068 works.\n\nHow can I assist your review today?",
      timestamp: 'Just now',
    },
  ]);
  ...
  const quickPrompts = [
    ...
    },
    {
      ...
      query: 'Analyze unusual vendor expenditure patterns without false positives',
      handler: () => {
        handleSendMessage('Explain vendor concentration risk and payment patterns');
      },
    },
    {
  ...
  const handleSendMessage = (customText?: string) => {
    ...
    setTimeout(() => {
      ...
      if (queryLower.includes('high risk') || queryLower.includes('critical') || queryLower.includes('composite')) {
      ...
      } else if (queryLower.includes('vendor') || queryLower.includes('concentration')) {
        aiText = `Vendor risk analysis monitors transaction pattern anomalies while minimizing false positives for legitimate single-vendor works.\n\n**Key Metrics Analyzed:**\n• Single-Vendor Disbursal Share (%)\n• Split Payment Clustering Score\n• Historical Vendor Execution Track Record\n• Sub-district Contracting Benchmark Difference`;
        actions = [
          {
            label: 'View Vendor & Disbursal Analytics',
            action: () => {
              onNavigateTab?.('vendor-analytics');
              onClose();
            },
          },
        ];
        ...
        aiText = `I have analyzed your query regarding "${textToSend}".\n\nThe MPLADS AI Risk Intelligence Platform evaluates works across 5 key dimensions:\n1. Composite Multi-Signal Risk Score\n2. Financial Disbursal Anomaly Model\n3. Vendor Share & Transaction Concentration\n4. Candidate Duplicate Vector Matching\n5. Compliance & Utilization Certificate Evidence Gaps.`;
        actions = [
          {
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl w-full max-w-2xl flex flex-col h-[650px] max-h-[90vh] overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500 dark:text-slate-400 font-medium">
                Decision Support Assistant for Works, Vendors & Expenditure Audit
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-3 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
          ...
          >
            <input
              type="text"
              placeholder="Ask AI Risk Assistant about works, financial anomalies, or vendors..."
              value={inputQuery}

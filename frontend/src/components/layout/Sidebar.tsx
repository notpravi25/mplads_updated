...
export const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab }) => {
  const sections = [
    ...
    },
    {
      ...
      items: [
        ...
        { id: 'vendor-analytics', label: 'Vendor & Disbursal Analytics', icon: Building2 },
  ...
  return (
    <aside className="w-64 bg-white dark:bg-[#0f172a] border-r border-slate-200/80 dark:border-slate-800 flex flex-col justify-between shrink-0 h-screen sticky top-0 shadow-[4px_0_24px_rgba(15,23,42,0.03)] z-20 transition-colors">
      <div className="flex flex-col h-full justify-between">
        <div>
          ...
          <nav className="p-3 space-y-4 overflow-y-auto max-h-[calc(100vh-145px)]">
            {sections.map((sec, secIdx) => (
              <div key={secIdx} className="space-y-1">
                <span className="px-3 text-[10px] uppercase font-extrabold text-slate-400 dark:text-slate-500 tracking-wider block mb-1">
                  {sec.title}
                </span>
                {sec.items.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center justify-between px-3 py-2 rounded-xl text-xs transition-all duration-200 text-left group ${
                        isActive
                    ...
                    >
                      ...
                      {isActive && <ChevronRight className="w-3.5 h-3.5 text-slate-400 dark:text-slate-500 shrink-0" />}
                    </button>
                  );
                })}
              </div>

...
The **MPLADS AI Risk Intelligence & Decision Support System (DSS)** is an enterprise-grade AI/ML analytical platform built to automate risk monitoring, duplicate candidate detection, financial anomaly analysis, vendor concentration tracking, compliance gap auditing, and schedule progress monitoring across India's national Member of Parliament Local Area Development Scheme (MPLADS) public works registry.
...
```
 ...
 CSV Administrative     Benford's Law, HHI Index      O(1) Hashmap Lookups     Night-View Design System
```
...
flowchart TD
    ...
    subgraph ML_Analytics ["2. ML Models & Feature Engineering Layer"]
        ...
        C --> D3[Herfindahl-Hirschman Vendor Index Engine]
        C --> D4[Schedule Delay & Progress Mismatch Engine]
        D1 & D2 & D3 & D4 --> E[MLflow Model Registry & Experiment Tracker]
    ...
    subgraph Frontend_UI ["4. React Executive Presentation Layer"]
        ...
        I --> J3[Financial Anomaly & Vendor Analytics]
...
2. **Feature Store Generation & Parquet Persistence**:
   - Computes feature vectors: Haversine spatial proximity ($<200\text{m}$), text n-gram TF-IDF vectors, Benford digit distributions, Herfindahl-Hirschman Index (HHI), schedule delay ratios, and compliance completeness points.
...
#### 3.1 Purpose & Role
Acts as the central multi-signal risk engine, aggregating risk outputs from deduplication models, financial anomaly algorithms, vendor risk metrics, compliance trackers, and schedule progress monitors.
...
$$R_{\text{composite}} = w_1 S_{\text{duplicate}} + w_2 S_{\text{financial}} + w_3 S_{\text{vendor}} + w_4 S_{\text{compliance}} + w_5 S_{\text{schedule}}$$

Where the feature weights are calibrated as:
- $w_1 = 0.25$ (Spatial Duplicate Candidate Score)
- $w_2 = 0.25$ (Financial Anomaly / Cost Escalation Score)
- $w_3 = 0.20$ (Vendor Concentration & Conflict Score)
- $w_4 = 0.15$ (Compliance Evidence Gap Score)
- $w_5 = 0.15$ (Schedule Delay & Progress Gap Score)
...
Features 6 filter views allowing auditors to isolate specific risk vectors:
...
3. `Vendor Risks`: Concentrated or high-risk implementing agencies.
4. `Duplicate Candidates`: Spatial proximity pairs within <200m.
5. `Compliance Gaps`: Missing UCs or geo-tagged photos.
6. `Schedule Delays`: Works with severe progress slippage.
...
### Module 6: Vendor & Disbursal Analytics (`VendorAnalyticsPage.tsx`)

#### 6.1 Purpose & Role
Monitors vendor selection fairness, detects monopolistic vendor concentration within districts, and identifies conflict-of-interest indicators.

#### 6.2 Key Analytics & Metrics
- **Herfindahl-Hirschman Index (HHI) for Vendor Concentration**:
  Measures market dominance of implementing agencies/contractors within a district:
  $$\text{HHI} = \sum_{i=1}^N \left( \frac{\text{Expenditure Disbursed to Vendor}_i}{\text{Total District Disbursed Expenditure}} \times 100 \right)^2$$
  - $\text{HHI} > 2,500$: **High Monopolistic Concentration Risk** (Single vendor capturing disproportionate works).
  - $1,500 \le \text{HHI} \le 2,500$: Moderate concentration.
  - $\text{HHI} < 1,500$: Healthy competitive allocation.

- **Rapid Disbursal Velocity Risk**:
  Detects instances where $>80\%$ of funds are released to a vendor within $<14\text{ days}$ of sanction without intermediate milestone verification.

---

### Module 7: Compliance Evidence Gaps (`ComplianceMonitorPage.tsx`)
...
### Module 9: Data Sync & System Status (`SystemStatusPage.tsx` & Backend `api/index.py`)
...
- **FastAPI Python Backend**: Exposes REST endpoints (`/api/summary`, `/api/risks`, `/api/duplicate-candidates`, `/api/vendor-analytics`).
...
### Module 10: MLflow Model Monitoring (`MlflowMonitoringPage.tsx`)
...
| **Vendor & Disbursal Analytics** | Monopolies & Vendor Risk | Herfindahl-Hirschman Index (HHI) | District HHI Score, Disbursal Velocity | $\text{HHI} > 2,500$ (High Concentration) |
